...
def submit_experiment(experiment, debug=False, cluster_params=None, **kwargs):
    ...
    if issubclass(experiment, DistributedExperiment):
        ...
        nodes, gpus_per_node, tasks_per_node = default_cluster_params_distributed(kwargs['path'])
        _cluster_params.update(nodes=nodes, gpus_per_node=gpus_per_node, tasks_per_node=tasks_per_node)
...
def default_cluster_params_distributed(path):
    ...
    assert world_size == nodes * tasks_per_node, \
